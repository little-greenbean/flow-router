...
这个 fork 目前集中在**把调度过程看清楚**这件事上——多网关、多路由、带顺延的调度，
出问题时最难回答的从来不是"失败率多少"，而是"这条请求到底走了哪条路、在哪一跳转的、
最后落在哪"。下面几项都是围绕这个问题做的。

### 请求流向桑基图

首页「调度情况」是一张请求流向图。默认按网关分流：左边是全部请求，中间是各个网关，
右边是三种结局——**一次过 / 顺延后成功 / 最终失败**。线有多粗就是有多少请求真的走了这条路，
红色的线是「这一跳失败后转走的」。

![请求流向总览](docs/images/dispatch-overview.png)

点上方的网关标签直接切换，下钻后按「第几跳打在哪条路由上」逐层铺开。第 1 跳是这个网关的
首选路由，往右每一列是一次顺延，最右边仍然是三种结局：

![单个网关的逐跳分流](docs/images/dispatch-drill.png)

图上每类节点都是一个动作：点路由跳到它的配置页，点右侧结局跳到使用记录并自动带上对应筛选
（比如点「最终失败」就是按失败筛过的那批请求）。细到几乎看不见的连线也能点——点线等于点它
流向的那个节点。

### 按顺延次数高亮完整链路

下钻之后，上方会出现按**顺延次数**分的高亮按钮，数字后面跟的是这一档实际有多少条请求链。
点一下不重新拉数据、也不重新铺图，只把「顺延了这么多次的请求，从进网关到出结局走过的每一段」
调亮、其余调暗：

![按顺延次数高亮完整链路](docs/images/dispatch-failover-highlight.png)

这里的「次数」指的是**一条请求从开始到结束**顺延了几次，不是某一跳的片段——所以高亮出来的
是完整路径，中间任何一跳都不会漏。需要注意线宽始终是「这条边总共多少请求」，高亮只改深浅
不改粗细（改粗细就得重新布局，那就是重新画一张图了），所以按钮上会把这一档的真实链数标出来。

### 悬停只解释，不改图

鼠标悬停只出 tooltip，不会去点亮别的链路。桑基图上一个节点同时属于很多条链，"我正悬停的
那条链"没有唯一解，让 hover 去猜只会把一堆不相干的链一起点亮，还会把点出来的高亮冲掉。

![悬停查看某一段的流量](docs/images/dispatch-tooltip.png)

图表跟随深浅色主题，切换即时生效：

![深色主题](docs/images/dispatch-dark.png)

### 路由可用性探测改走流式

模型探测以前发的是非流式请求，等整条响应生成完才算数。中转（尤其带缓存的 Claude 线路）
首字只要 2~3 秒，整条却经常十几秒、峰值上百秒——于是一批**实际可用**的路由被判成
`context deadline exceeded`，看着跟真挂了一样。

现在探测发流式请求、拿到第一个字节就算这条路通，跟客户端的真实使用方式同一个口径；
首字预算跟网关组配置的 `first_token_timeout_sec` 走，没配用默认值。界面上显示的耗时也
相应改成「首字 xx ms」。

### TODO：引入 AI 辅助调度

当前调度器仍是确定性的规则调度。后续计划把 AI 作为决策层引入：评估路由健康度、延迟、
失败率与成本，动态调整路由权重。现有规则与顺延链路会保留为基线和兜底，避免 AI 决策
成为单点。
...
新版 NewAPI（QuantumNous/new-api 及其分叉）登录后改用短期 `access_token`（Bearer JWT，默认 15 分钟）鉴权，并随响应下发 `new_api_refresh` 刷新令牌。Flow Router 会在令牌临近过期时自动调用 `/api/user/auth/refresh` 续期并轮换刷新令牌，无需频繁重新登录；老版本 NewAPI 仍走 `Set-Cookie: session=...` + `New-Api-User` 头鉴权，由程序自动识别兼容。
