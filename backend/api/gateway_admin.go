...
func errorsGatewayDispatch(c *gin.Context, d *Deps) {
	if d.GatewayUsage == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "usage storage unavailable"})
		return
	}
	...
	if !to.After(from) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid error range"})
		return
	}
	errors, err := d.GatewayUsage.DispatchErrors(from, to)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
