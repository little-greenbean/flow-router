...
func (r *GatewayUsageLogs) DispatchFlow(from, to time.Time, groupID uint, failoverFilter int) (GatewayDispatchFlow, error) {
	...
	maxObservedHops := 0
	for _, key := range chainOrder {
		hops := len(chains[key].attempts)
		attempts += hops
		if hops > maxObservedHops {
			maxObservedHops = hops
		}
	}
	result.Attempts = attempts
	result.MaxHops = maxObservedHops
	...
	// 结局列紧跟在实际用到的最深一跳后面，不按理论上限（dispatchFlowMaxHops）留位置——
	// 这个网关这一窗口如果最深只顺延到第 4 跳，结局列就该紧挨着第 4 跳，而不是永远
	// 空出到第 7 列，把中间跳数据全部挤扁、右边留一大片没用的空白。
	outcomeDepth := 2
	if groupID > 0 {
		outcomeDepth = maxObservedHops + 1
		if maxObservedHops > dispatchFlowMaxHops {
			outcomeDepth = dispatchFlowMaxHops + 2 // 超限的链会额外占一列「更深跳以后」
		}
		if outcomeDepth < 2 {
			outcomeDepth = 2
		}
